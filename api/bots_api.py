...
class BotParameters(BaseModel):
    ...
    positionSize: float = Field(..., ge=0.01, le=1.0, description="Position size as fraction")
    riskLevel: str = Field(..., description="Risk level: conservative, moderate, aggressive")
...
def convert_backend_to_frontend_params(backend_params: Dict[str, Any]) -> BotParameters:
    ...
    return BotParameters(
        ...
        positionSize=backend_params.get("base_trade_size", 0.1),
